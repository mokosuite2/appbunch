...
static Evas_Object* make_dialer(void)
{
    ...
    elm_layout_file_set(dialer, MOKOSUITE_DATADIR "theme.edj", "mokosuite/phone/dialer");
    ...
    elm_layout_file_set(obj_keypad, MOKOSUITE_DATADIR "theme.edj", "mokosuite/common/dialpad");
...
gboolean phone_win_call_internal(const char *peer, gboolean *hide)
{
...
void phone_win_ussd_reply(int mode, const char* message)
{
    ...
    if (!ecore_x_window_visible_get(xwin) || ecore_x_window_focus_get() != xwin) {
        g_debug("Notifying USSD response to panel");
        if (ussd_notification_id)
            moko_notifications_remove(panel_notifications, ussd_notification_id, NULL);
...
void phone_win_init(void)
{
    ...
    mokowin_create_layout(win, MOKOSUITE_DATADIR "theme.edj", "mokosuite/phone/main");
