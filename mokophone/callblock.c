...
static void configure_call(PhoneCallBlock* call, gboolean outgoing);
static void create_bubble(PhoneCallBlock* call, gboolean outgoing);
static void init_incoming(PhoneCallBlock* call);
static void init_outgoing(PhoneCallBlock* call, gboolean initiate);

static gboolean insert_call(gpointer data);
static void insert_call_post(PhoneCallBlock* call);

static void end_call(PhoneCallBlock* call, gboolean release);
...
static void configure_call(PhoneCallBlock* call, gboolean outgoing)
{
    if (outgoing) {
        ...
        elm_layout_file_set(call->layout, MOKOSUITE_DATADIR "theme.edj", "mokosuite/phone/call/active");

        /* icona pulsante hold */
        Evas_Object *icon_hold = elm_icon_add(call->parent->win);
        elm_icon_file_set(icon_hold, MOKOSUITE_DATADIR "call-hold.png", NULL);
        ...
        elm_icon_file_set(icon_release, MOKOSUITE_DATADIR "call-end.png", NULL);
        ...
        elm_layout_file_set(call->layout, MOKOSUITE_DATADIR "theme.edj", "mokosuite/phone/call/incoming");
...
void phone_call_block_call_status(PhoneCallBlock* call, const CallStatus status, GHashTable* properties)
{
    ...
    switch(status)
    {
        ...
        case CALL_STATUS_ACTIVE:
            ...
            gboolean create_time_idler = FALSE;
...
PhoneCallBlock* phone_call_block_new(MokoWin* parent, const char* peer, int id, gboolean outgoing)
{
