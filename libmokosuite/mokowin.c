...
static void _close(void *mokowin, Evas_Object *obj, void *event_info)
{
    ...
    if (mw->inners->len > 0) {
        // rimuovi la inwin -- l'oggetto sara' distrutto automaticamente
        g_ptr_array_remove_index(mw->inners, mw->inners->len - 1);
        return;
    }

    if (mw->menu_hover != NULL) {
        g_debug("Hover visible: %d", (int)evas_object_visible_get(mw->menu_hover));
...
static void _key_down(void *mokowin, Evas *e, Evas_Object *obj, void *event_info)
{
    ...
    g_debug ("Keyboard(down): %s, %u, %d", event->keyname, event->timestamp, (int) (event->timestamp - mw->last_keystamp));
    mw->key_down = TRUE;
}
...
static void _key_up(void *mokowin, Evas *e, Evas_Object *obj, void *event_info)
{
    ...
    g_debug ("Keyboard(up): %s, %u, %d", event->keyname, event->timestamp, (int) (event->timestamp - mw->last_keystamp));
    ...
    if ((!strcmp(event->keyname, MENU_KEY_PRIMARY1) ||
        ...
        if (mw->inners->len == 0 && mw->menu_enable) {
...
{
    g_return_if_fail(mw->vbox == NULL || mw->layout == NULL);
...
void mokowin_menu_set(MokoWin *mw, Evas_Object *box)
{
    ...
    g_return_if_fail(mw->menu_hover != NULL);
...
{
    // controlla se non abbiamo gia' scelto il layout
    g_return_if_fail(mw->layout == NULL);
...
void mokowin_scroll_freeze_set_callback(ScrollCallbackData *data)
{
    ...
    g_return_if_fail(data->win != NULL);
...
{
    g_return_if_fail (mw->scroller != NULL);
...
{
    MokoWin *mw = g_malloc0(size);

    /* costruisci la finestra */
    mw->win = elm_win_add(NULL, name, type);

    if(mw->win == NULL) {
        g_warning("Cannot instantiate window");
        g_free(mw);
        return NULL;
    }

    mw->inners = g_ptr_array_new_with_free_func((GDestroyNotify)mokoinwin_destroy);
    ...
    if (!evas_object_key_grab(mw->win, MENU_KEY_PRIMARY1, 0, 0, FALSE))
        g_warning("Unable to grab primary1 key %s", MENU_KEY_PRIMARY1);

    if (!evas_object_key_grab(mw->win, MENU_KEY_PRIMARY2, 0, 0, FALSE))
        g_warning("Unable to grab primary2 key %s", MENU_KEY_PRIMARY2);

    if (!evas_object_key_grab(mw->win, MENU_KEY_SECONDARY, 0, 0, FALSE))
        g_warning("Unable to grab secondary key %s", MENU_KEY_SECONDARY);
