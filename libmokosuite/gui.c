...
MokoPopupAlert* moko_popup_alert_new(MokoWin *parent, const char *label)
{
    ...
    MokoPopupAlert *p = g_realloc(inwin, sizeof(MokoPopupAlert));
...
MokoPopupStatus* moko_popup_status_new(MokoWin *parent, const char *status)
{
    ...
    MokoPopupStatus *p = g_realloc(inwin, sizeof(MokoPopupStatus));
    p->parent = parent;
...
{
    g_return_if_fail(popup != NULL && label != NULL);
...
MokoPopupMenu* moko_popup_menu_new(MokoWin *parent, const char *message, MokoPopupMenuStyle style, gpointer callback, gpointer data)
{
    ...
    MokoPopupMenu *p = g_realloc(inwin, sizeof(MokoPopupMenu));
    p->parent = parent;
    p->style = style;
    p->callback = callback;
    p->data = data;
...
static void _pslider_close(gpointer inwin)
{
...
static void _pslider_change(void* data, Evas_Object* obj, void* event_info)
{
    ...
    g_debug("Slider changed: %d", p->value);
...
MokoPopupSlider* moko_popup_slider_new(MokoWin *parent, const char *message, int value, gpointer callback, gpointer data)
{
    ...
    MokoPopupSlider *p = g_realloc(inwin, sizeof(MokoPopupSlider));
    p->parent = parent;
    p->callback = callback;
    p->data = data;
